...
<template>
    <div
        ...
        <div v-if="isOpen" class="absolute right-0 mt-2 mr-2 w-48 bg-white border border-gray-300 rounded shadow-lg">
            <ul>
                <li v-for="(link, index) in links" :key="index">
                    <router-link :to="link.to" @click="isOpen = !isOpen"
                        class="block px-4 py-2 text-gray-800 hover:bg-gray-100">
                        {{ t(link.translationKey || link.name) }} <!-- Use translation key or fallback to name -->
                    </router-link>
                </li>
            </ul>
        </div>
